...
function ChatRoomContent() {
  ...
  const [mode, setMode] = useState<"research" | "verify">("research"); // Kept for theme compatibility but hidden
  ...
  const handleUserSubmit = async (text: string) => {
    ...
    const newMsg: Message = { id: userId, role: "user", type: "text", content: text };
    ...
    try {
      ...
      let activeThoughtId = `${Date.now()}-thought`;
      let activePapersId = `${Date.now()}-papers`;
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (type === "thought" || type === "error") {
              // Update or replace the latest assistant thought
              setMessages((prev) => {
              ...
              // Auto-activate Workstation Mode on first row
              ...
              setMessages((prev) => {
                ...
                return [...filtered, {
                  ...
                  role: "assistant",
                  type: "matrix",
                  content: "Deep Synthesis Matrix (Current Session)",
                  matrix: currentMatrix
                }];
              });
            } else if (type === "text") {
              // Final conclusion: Clear the technical thought log to keep it professional
              setMessages((prev) => {
                const filtered = prev.filter(m => m.id !== activeThoughtId);
                return [
                  ...filtered,
  ...
  return (
    <div className={`relative flex h-screen bg-slate-950 text-white font-sans overflow-hidden transition-colors duration-700 ${mode === "research" ? "selection:bg-indigo-500/30" : "selection:bg-amber-500/30"}`}>
      ...
      <div className="fixed inset-0 z-0 [background-size:40px_40px] [background-image:linear-gradient(to_right,#262626_1px,transparent_1px),linear-gradient(to_bottom,#262626_1px,transparent_1px)] pointer-events-none opacity-40"></div>
      <div className="fixed inset-0 z-0 bg-slate-950 [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)] pointer-events-none"></div>
      ...
      <main className="relative z-10 flex-1 flex transition-all duration-700 overflow-hidden">
        ...
        <div className={`flex flex-col h-full border-r border-slate-800/10 transition-all duration-700 ${isWorkstationMode && !isMaximized ? "w-[35%] opacity-90" : isMaximized ? "w-0 opacity-0 pointer-events-none" : "w-full"}`}>
          <header className="border-b border-slate-800/60 bg-slate-950/60 backdrop-blur-md p-4 flex items-center justify-between sticky top-0 z-30">
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} w-full`}>
                  <div className={`transition-all duration-500 ${msg.role === "user" ? "p-4 rounded-3xl bg-indigo-600 shadow-xl shadow-indigo-600/10 max-w-[80%]" : "w-full"}`}>
                    ...
                    {(msg.type === "loading" || (msg.role === "assistant" && msg.type === "text" && !msg.content?.includes("Đã hoàn thành"))) && msg.content && (
                      <AgentThoughtProcess
                        thoughts={msg.content}
                        isCollapsed={msg.type !== "loading"}
                      />
                    )}
                    ...
                    {msg.type === "papers" && (
                      <div className="space-y-6 bg-slate-900/40 p-6 rounded-[2.5rem] border border-slate-800">
                        <div className="flex items-center gap-3">
                          <div className="p-2 bg-indigo-500/10 rounded-full"><Search className="w-4 h-4 text-indigo-400" /></div>
                          <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{msg.content}</div>
                        </div>
                        <div className="grid grid-cols-1 gap-3">
                          {msg.papers?.slice(0, 3).map(p => (
                            <div
                              key={p.id}
                              onClick={() => { setActiveDetail({ title: p.title, content: p.abstract }); setShowDetail(true); }}
                              className="group/paper p-4 bg-slate-950/40 hover:bg-slate-900 border border-slate-800 rounded-2xl transition-all cursor-pointer"
                            >
                              <div className="font-bold text-slate-100 line-clamp-1 group-hover/paper:text-indigo-400 transition-colors">{p.title}</div>
                              <div className="text-[9px] text-slate-500 mt-1 uppercase font-mono">{p.authors}</div>
                            </div>
                          ))}
                        </div>
                        <button
                          onClick={() => msg.papers && executeExtraction(msg.papers.slice(0, 5))}
                          className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-600/20 active:scale-[0.98]"
                        >
          ...
          <div className="p-6 relative bg-transparent">
            ...
            >
              ...
              <input
                ...
                className="w-full bg-slate-900/80 backdrop-blur-2xl border border-slate-800 rounded-full px-7 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/40 transition-all shadow-2xl relative z-10"
              />
              <button className="absolute right-2.5 top-1/2 -translate-y-1/2 p-2.5 bg-indigo-600 rounded-full text-white shadow-lg hover:bg-indigo-500 hover:scale-110 active:scale-95 transition-all z-20">
        ...
        {isWorkstationMode && (
          <div className={`flex flex-col h-full bg-slate-900/40 backdrop-blur-3xl transition-all duration-700 ease-in-out relative ${isMaximized ? "w-full" : "w-[65%] border-l border-slate-800"}`}>
            <header className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/40 sticky top-0 z-20">
              <div className="flex items-center gap-3">
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-100">Synthesis Workstation</span>
              </div>
              <div className="flex items-center gap-2">
                <button
